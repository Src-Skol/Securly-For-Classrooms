...
function I() {
  ...
  e.value.trim().length > 0 &&
    (chrome.runtime.sendMessage({ [c]: o, [s]: l, [u]: e.value }),
    q(e.value, !1, "You", new Date().toUTCString()),
    (e.value = ""));
}
function b(e, n) {
...
function w(e, t) {
  chrome.runtime.sendMessage({ [s]: n, isConfirmed: e, userId: t.userId });
}
function C(e) {
  (13 !== e.keyCode && 3 !== e.keyCode) || (e.preventDefault(), I());
}
function S() {
  const t = { [s]: l, [c]: e };
  chrome.runtime.sendMessage(t, (e) => {
    console.log("Response=" + JSON.stringify(e)),
      (function (e) {
        ...
        e.forEach((e) => q(e[u], e[p], e[p] ? e[g] : "You", e[m])), j();
      })(e[i]),
...
function q(e, t, n, o) {
  ...
  (s = (s = (s = (s = t
    ? s.replace(/@receiver@/g, "toMe")
    : s.replace(/@receiver@/g, "mine")).replace(
    "@time@",
    (function (e) {
      const t = new Date(e),
        n =
      ...
      return o + ":" + s + ":" + a + (t.getHours() - 12 < 0 ? " AM" : " PM");
    })(o),
  )).replace("@text@", O(e))).replace("@name@", n)),
    document.getElementById("tpMessageList").insertAdjacentHTML("beforeend", s),
    j();
}
function O(e) {
  return e
    ...
    x("tpIcofont", "icofont.css")
      ...
      .then(() =>
        (async function (e, t) {
          ...
          console.log("HTML " + e + " injected"), (M = new D());
      ...
      .then(() => {
        ...
        (o = "tp_blk_icon"),
          (i = t),
          (document.getElementById(o).src = chrome.runtime.getURL(i)),
          (document.getElementById("tpCloseBtn").onclick = T);
        const p = e[a];
        if ("string" == typeof p) {
        ...
        if (
          ("string" == typeof y &&
            ((document.getElementById("tpMsgTable").style.display = "table"),
            (document.getElementById("tpConfirmTable").style.display = "none"),
            (document.getElementById("blocked_message").innerHTML = y),
            ...
            h ||
              ...
              (function () {
                let e = 0,
                  ...
                  n = 0,
                  o = 0,
                  s = document.getElementById("tpMovableIcon");
                ...
                function i(a) {
                  a.preventDefault(),
                    ...
                    (n = a.clientX),
                    (o = a.clientY),
                    (s.style.top = s.offsetTop - t + "px"),
                    (s.style.left = s.offsetLeft - e + "px");
                }
                function d(e) {
                  document.removeEventListener("mouseup", d),
                    document.removeEventListener("mousemove", i),
                    Math.abs(a - e.clientX) < 5 &&
              ...
              (document.getElementById("tpSendButton").onclick = I),
              (document.getElementById("tpText").onkeyup = C),
              (h = !0));
            const t = e.chatMessage,
              n = !!e.forceCloseChat,
              o = !!e.forceOpenChat;
            n && A()
              ? (console.log("Close chat"), T())
              : A()
              ? t && q(t[u], !0, t[g], t[m])
              : o
              ? (H(), S())
        ...
        if ("tabCloseWarning" === e[c]) {
          E([a, r]),
            (v = e.delaySeconds),
            (document.getElementById("tpMsgTable").style.display = "table"),
            (document.getElementById("tpConfirmTable").style.display = "none"),
            (document.getElementById("tpAnnouncementMessage").innerHTML = B(v)),
            (document.getElementById("tpAnnouncementCover").style.display =
              "flex");
          ...
          document.getElementById("tpOkButton").onclick = function () {
            clearInterval(t),
              chrome.runtime.sendMessage({ [s]: "clearTabsCloseWarning" });
          };
        }
        "requestShareScreen" === e[c] &&
          (E([a, r, d]),
          ...
          "requestVideoCall" === e[c] &&
            ...
            (document.getElementById("tpRequestConfimTitle").innerHTML =
              "Call from teacher"),
            (document.getElementById("tpRequestConfimMessage").innerHTML =
              "Teacher started 1-on-1 video call"),
            (document.getElementById("tpAgreeButton").innerHTML = "Join"),
            (document.getElementById("tpRejectButton").innerHTML = "Decline"),
            (document.getElementById("tpAgreeButton").onclick = function () {
              w(!0, e);
            }),
            (document.getElementById("tpRejectButton").onclick = function () {
...
class D {
  ...
  generateElements(e) {
    const t = (t) => {
      ...
      n = (e) => {
        (this.stopClickEvent = !0), (s.style.display = "block");
        for (var t = s.querySelectorAll("ul"), n = 1, o = t.length; n < o; n++)
          t[n].style.display = "none";
        var a = s.querySelectorAll(".category div");
        for (n = 0, o = a.length; n < o; n++) a[n].style.background = "none";
        (s.querySelector("." + e.target.id).style.display = "block"),
          (s.querySelector("#" + e.target.id).style.background = "#c2c2c2");
      };
    e.style.width = "100%";
    ...
    (a.style.position = "absolute"),
      ...
      (a.onclick = () => {
        "none" === s.style.display
          ? (s.style.display = "block")
          : "block" === s.style.display && (s.style.display = "none"),
          s.focus();
      }),
      e.hasAttribute("data-meteor-emoji-large") || o.appendChild(a);
    ...
    g.map((e) => {
      ...
      window.addEventListener("click", function (t) {
        if (!e.hasAttribute("data-meteor-emoji-large")) {
          if (M.stopClickEvent) return void (M.stopClickEvent = !1);
